      ...
      <div>
        ...
        <div className="flex flex-col gap-1.5">
          {MOCK_CATEGORIES.map((cat) => {
            const isSelected = selectedCategories.includes(cat.slug.current);
            return (
              <label
                key={cat._id}
                className={cn(
...
function ActiveChips({
  ...
  selectedCategories.forEach((slug) => {
    const cat = MOCK_CATEGORIES.find((c) => c.slug.current === slug);
    if (cat) {
      chips.push({
        key: `cat-${slug}`,
        label: cat.name,
...
function BrowseInner() {
  ...
  const filteredUseCases = useMemo(() => {
    ...
    return results;
  }, [searchQuery, selectedCategories, selectedComplexity, sort]);
